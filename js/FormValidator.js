export class FormValidator{
  constructor(settings_object, form){
    this._settings_object = settings_object;
  ...
  _toggleButtonState(inputList, buttonElement) {
    if (this._hasInvalidInput(inputList)) { // если в списке есть невалидные элементы
      ...
      buttonElement.classList.add(this._settings_object.inactiveButtonClass); // изменяю вид кнопки
    } else {                                // все элементы списка валидные
      buttonElement.removeAttribute("disabled"); // убираю выключение
      buttonElement.classList.remove(this._settings_object.inactiveButtonClass); // изменяю вид кнопки
    }
  }
  ...
  _hideInputError(formElement, inputElement) {
    ...
    errorElement.classList.remove(this._settings_object.errorClass);
    inputElement.classList.remove(this._settings_object.inputErrorClass);
  }

  //показать ошибку
  _showInputError(formElement, inputElement) {
    const errorElement = formElement.querySelector(`.${inputElement.id}-error`);
    errorElement.classList.add(this._settings_object.errorClass);
    inputElement.classList.add(this._settings_object.inputErrorClass);
  ...
  enableValidation(){
    ...
    const inputList = Array.from(this._formElement.querySelectorAll(this._settings_object.inputSelector));
    //кнопка "Записать"
